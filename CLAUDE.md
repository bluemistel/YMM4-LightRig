...
### リライティングは乗算なので「受光量」なしでは明るくならない（2026-08・実機）
`relit = rgb * tone`（`tone` は影色→光色の補間）という**乗算**なので、
光色が最大 1 のままでは<b>結果が必ず元画素以下</b>になる。つまり既定では
「色が付いて暗くなる」だけで、シェーディングの擬似ノーマルと見た目が近くなる。
加算項はハイライト（`pow(ndl, shininess)`）だけで、これは狭い光沢なので面を照らさない。

- **たき火を立ち絵の前面に置くような「光を受けて明るくなる」表現には光色を 1 超にする必要がある。**
  そのための `受光量`（既定100%＝従来と同じ）を C# 側に持たせ、`effLight` へ掛ける。
- **1 を超えたら出力を `saturate` してから premultiply すること。**
  `rgb > a` はプリマルチプライドとして不正で、半透明の髪の縁などで合成が破綻する。
- なお LightRig には**光源色を面全体へ加算するエフェクトが無い**（リムは輪郭のみ、
  背景なじませは背景色由来）。受光量で足りるかは実機で判断すること。
