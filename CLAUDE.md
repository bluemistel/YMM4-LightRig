...
- **【発信値には必ずフレームを刻み、消費側は同フレームを優先する（2026-08・実機の不具合）】**
  当初の選択は「同じ Usage → 無ければ最新 Seq」だけだった。この場合、再生を止めて
  YMM4 が**別 Usage で描き直した瞬間**に自 Usage のエントリが無く最新 Seq へ落ち、
  **再生中の最後に発信された「別の時刻」の値**を拾ってしまう。
  実機では「夕方のシーンから真夜中へ移って停止すると、夕方の環境光が乗ったまま固まる」
  （アイテムを触ると再描画されて直る）という形で出た。全消費エフェクトで同時に起きる。
  `LightState.Frame` / `AmbientState.Frame` に発信時のフレームを刻み、
  優先順位を**「同 Usage かつ同フレーム → 同フレーム(別 Usage) → 同 Usage → 最新 Seq」**にする。
