...
**対策（自動）**: **`isHeld` の値は有効範囲を一切問わない**（`Entry.IsUsableAt`）。
`isHeld` は「終わったアイテムが場面切り替えのために意図的に描画されている」状態で、
通常の再生では起こらないため、これを根拠にしてよい。

**【重要・当初の実装は間違いだった（2026-09・エンコードでのみ再現）】**
最初は `validTo = frame + 1` として「現在フレームだけ」有効範囲を伸ばしていた。
これだと**各エントリが1フレームしか有効にならない**ため「最も近いフレーム」の
フォールバックが機能せず、**同一フレーム内で発信側が消費側より先に走ったかどうかの運任せ**になる。
- **プレビューは同じフレームを何度も描くので履歴が埋まり、数回再生すると直ってしまう。**
  そのため「直った」と誤認しやすい。
- **1パスしか描かないエンコードでは半分の確率で外れ**、立ち絵が固定色のまま白く出る。
- 症状が「プレビューでは直るのにエンコードで出る」なら、まず
  **フレーム履歴に依存していないか**を疑うこと。1パスで成立するかを必ず考える。

**保持中の値を使えるのは保持中の描画だけ**に限ること（`IsHeld ? consumerHeld : CoversFrame`）。
範囲を問わないので、限定しないと切り替え後も前の場面の値が永久に拾われる。

**編集検出（`changeComparer`）は場面切り替え中に誤爆する。** 前後2本は同じアイテムを
発信元キーとして共有するので、同じフレームへ違う値（環境光追従で前後の背景色が違う等）を
書き合う。これを編集と誤認して履歴を捨てるとエンコードで値が消える。
**側（`IsHeld`）が一致するときだけ編集と見なす。**
- 【当初は手動パラメータ「発信の延長」を置いたが、`RenderSide` の導入で自動化できたため削除した】
  利用者が切り替えの長さを手入力する必要は無い。
- たき火が普通に終わった場合は `isHeld` にならないので、光が残らない修正はそのまま維持される。
- アイテム自体を切り替え分だけ伸ばしても同じ効果になる（そちらの置き方でも動く）。

### 場面切り替え中は「プロセッサが作り直される」（2026-09・実機／IL 調査済み）
症状: 発信の延長で切り替え**前**の場面は直ったのに、切り替え**後**の場面が
切り替え中だけなじまない。次の場面のアイテムは開始済みなので有効範囲の問題ではない。

**IL から確認した追加の事実:**
- `TransitionSource.ctor` は `TransitionItemPicker` と `TimelineSource` を**2組生成する**
  （`isBefore` フラグ違い）。`IGraphicsDevices.CreateContext()` もそれぞれに作る。
- つまり**場面切り替えが始まると、前後それぞれで全アイテムの
  `IVideoEffectProcessor` が新規に作り直される**。切り替えが終われば破棄される。
- `TransitionItemPicker.PickItems` は `isBefore` のとき**アイテム選択のフレームを
  場面切り替えアイテムの開始フレームに固定する**。時刻自体は前後とも現在フレーム。
  だから前側は「終わったはずのアイテムを現在時刻で描く」ことになり、発信の延長が要る。

**ここから来る不具合**: 環境光サンプラーは測定結果をプロセッサのフィールドに持っていたため、
切り替え開始時に**測定値ゼロの新品**になる。`_hasColor == false` の状態で初回サンプリングが
失敗すると `Publish` 自体が走らず、**何も発信しないまま切り替えが終わる**。
消費側は固定色へフォールバックし、立ち絵だけ浮いて見える。
さらに失敗時の待ちが初回から 60 フレーム（30fps で2秒）だったので、
一度失敗すると切り替え中はまず復帰できなかった。

**対策**:
- 測定結果を `ConditionalWeakTable<AmbientSamplerEffect, SampleCache>` で
  **アイテム単位**に持つ。作り直された側が直前の測定値をそのまま引き継げる。
  発信元キーを `item` にしているのと同じ理由（M11）。
- 前後2本は**並列に Update される**ので、測定は `Lock` で直列化する。
  同じアイテム＝同じ絵なので、片方が測れば他方は測り直さなくてよい（負荷も半分）。
- **失敗時の再挑戦は最初を短くする。** `1 << min(count,6)` フレーム（2,4,8…64）にした。
  一時的な失敗から即座に復帰しつつ、恒常的な失敗では例外の連発も避けられる。

**【重要・以前の記述の訂正】前後の判別は可能。`Shared/RenderSide.cs` に実装した。**
「アイテムが終わった」と「切り替え中で保持されている」は区別できないと書いたが、
**`ItemPosition.Frame >= ItemDuration.Frame`（自分の終端より後ろを描かされている）**は
切り替えの前側でしか起こらない。通常の再生では終わったアイテムはそもそも描画されないため。

### 前後の取り違えを防ぐ（2026-09・実機の不具合）
症状: 切り替えが始まった瞬間に、**前の場面にいる立ち絵が次の場面の環境光・光源を拾う**。
発信の延長を設定しても解消しない。

前後2本は **SceneId・Usage・TimelinePosition がすべて同じ**（説明オブジェクトが同一インスタンス）なので、
同じ `(SceneId, Channel)` の同じフレームに**前の場面と次の場面の値が同居する**。
どちらを採るかを決める情報が説明オブジェクト側に無い。
`EffectDescription.GetCustomValue` にも切り替えの印は入らず、`devices` は
`EffectedItemSource.ctor` が**アイテムごとに** `CreateContext()` するので側の識別には使えない。

**対策**: 発信値に `IsHeld`（`RenderSide.IsHeld`）を持たせ、消費側も自分の `IsHeld` を判定して
**同じ側どうしを優先**して結び付ける。見つからなければ側を問わず拾うフォールバックを残すので、
以前 true を返していた場面で false にはならない。

- **切り替えをまたいで存在するアイテムは前後どちらでも「保持されていない」**ため見分けられない。
  そのケースは次の場面側の値が優先される。**原理的な限界**なので、
  厳密に分けたいときは立ち絵側も切り替え位置でアイテムを分割してもらう。
- `FindSlot` は側指定つきのオーバーロードを用意し、
  `FindSlot(signals, frame, isHeld) ?? FindSlot(signals, frame)` の順で探す。
...
- 通知済みのタグを `%LOCALAPPDATA%\LightRig` に記録し、同じバージョンを繰り返し知らせない。
  **記録するのは「もう知らせなくてよい」と利用者が選んだときだけ。**
  「いいえ」で閉じた場合は次回も知らせるかを続けて訊く（訊かずに記録すると、
  後で入れようと思っていた利用者が次のバージョンまで古いまま放置される）。
  ダイアログを出せなかった場合（`Application.Current` が null 等）も記録しない。
...
### 落とし影は伸ばす向きを選ばせる（2026-09・実機の要望）
帯モデルは接地線から**画面の上へ**伸ばす前提だった（＝影が奥へ落ちる＝光源が被写体より手前）。
逆光（光源が被写体より奥）では影がカメラ側へ落ちるので、**下へ伸ばす**必要がある。

- **自動判別はできない。** シーン光源は2D位置しか持たず、`Height` は影の長さに使う量であって
  奥行きではない。「上にある光源」が手前なのか奥なのかを区別する情報がどこにも無い。
  利用者に選ばせる（`ShadowDirection`）。背景の建物や木の影の向きに合わせてもらう。
- 実装は `flip`（+1=奥／-1=手前）1つで済む。
  `hOut = (groundY - posScene.y) * flip` とすれば **hOut はどちらの向きでも正の距離**になり、
  足元 t=0・頭 t=1 の対応も `lean` の符号もそのまま成立する。
- **`lean` は反転しない。** 影は常に光の反対側へ倒れるので `-dir.X` のままでよい。
- **矩形の拡張は上下が入れ替わる。** 帯の占める範囲を
  `flip>0: [groundYRel - shadowLength, groundYRel]` /
  `flip<0: [groundYRel, groundYRel + shadowLength]` と分けて求め、
  `above = max(-bandTop, 0)` / `below = max(bandBottom - imageHeight, 0)` にする。
  ここを従来のままにすると手前向きの影が丸ごと見切れる。
- **`flip` の既定値は cbuffer の 0 ではダメ。** 0 だと `hOut` が常に 0 になり、
  帯の判定 `0 <= hOut <= shadowLength` が全画素で真になって足元の行が画面中に広がる。
  `EffectImpl` のコンストラクタで `_cb.Flip = 1f` を入れておく。

### 【HLSL ファイルに BOM を付けてはいけない（2026-09）】
`fxc.exe` が `error X3000: Illegal character in shader file` を (1,1) で出す。
既存の `Shaders\*.hlsl` はすべて **BOM 無し UTF-8**。
`.cs` は BOM 付きなので、スクリプトで一括編集するときにエンコーディングを取り違えやすい。
